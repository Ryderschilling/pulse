...
// Search Console history for storage: one row per day, plus the top 50
// queries per day. Google only keeps 16 months, Pulse keeps it forever.
export async function gscDaily(property, from, to) {
  const base = { startDate: from, endDate: to };
  const [daily, byDayQuery] = await Promise.all([
    gscQuery(property, { ...base, dimensions: ["date"], rowLimit: 5000 }),
    gscQuery(property, { ...base, dimensions: ["date", "query"], rowLimit: 5000 }),
  ]);
  const days = (daily.rows || []).map((x) => ({ day: x.keys[0], clicks: x.clicks, impressions: x.impressions, ctr: x.ctr, position: x.position }));
  const perDay = {};
  for (const x of byDayQuery.rows || []) {
    const [day, query] = x.keys;
    (perDay[day] ||= []).push({ query, clicks: x.clicks, impressions: x.impressions, position: x.position });
  }
  const queries = [];
  for (const day of Object.keys(perDay)) {
    perDay[day].sort((a, b) => b.clicks - a.clicks || b.impressions - a.impressions);
    for (const r of perDay[day].slice(0, 50)) queries.push({ day, ...r });
  }
  return { days, queries };
}
...
// ---------- GA4 daily history (for backfill and the daily cron) ----------
// Exact event names only. The old loose regexes counted `form_start` (someone
// clicked into a field) as a form lead, which is how 4 of the first 5 "forms"
// in the database never happened. Anything not on this list is ignored, and a
// site can add its own names in Settings as "event=bucket" pairs.
export const GA4_LEAD_EVENTS = {
  call: ["call_click", "phone_click", "click_to_call", "tel_click", "phone_call", "call", "phone_tap", "call_button_click"],
  form: ["form_submit", "lead_form_submit", "generate_lead", "contact_form_submit", "form_submission", "submit_form", "contact_submit", "quote_request", "lead"],
  email: ["email_click", "mailto_click", "email_tap", "email"],
  booking: ["booking_click", "book_now", "book_now_click", "schedule_click", "calendly_click", "appointment_booked", "booking", "schedule_call", "book_appointment"],
};
const NEVER = /(_start|_view|_error|_impression|_abandon|_progress)$|^form_start$/i;

export function leadMap(site) {
  const map = {};
  // Emails count as forms (Ryder, 09-21), so the email bucket folds into form.
  for (const [bucket, names] of Object.entries(GA4_LEAD_EVENTS)) for (const n of names) map[n] = bucket === "email" ? "form" : bucket;
  const extra = String((site && site.ga4_lead_events) || "");
  for (const pair of extra.split(/[,\n]/)) {
    const [name, bucket] = pair.split("=").map((s) => (s || "").trim());
    if (!name) continue;
    if (["call", "form", "email", "booking"].includes(bucket)) map[name] = bucket === "email" ? "form" : bucket;
    else if (bucket === "ignore" || bucket === "") delete map[name];
  }
  return map;
}

export async function ga4Daily(propertyId, from, to, site) {
  const map = leadMap(site);
  const range = { dateRanges: [{ startDate: from, endDate: to }] };
  ...
  for (const r of rowsToObjects(events, ["date", "eventName"], ["eventCount"])) {
    seen[r.eventName] = (seen[r.eventName] || 0) + r.eventCount;
    const bucket = map[r.eventName];
    if (!bucket || NEVER.test(r.eventName)) continue;
    const day = fmt(r.date);
    const col = bucket + "s";
    (days[day] ||= { day, users: 0, sessions: 0, pageviews: 0, engaged: 0, calls: 0, forms: 0, emails: 0, bookings: 0 })[col] += r.eventCount;
  }
  // Names GA4 saw that Pulse did not count, so Settings can show them and the
  // user can map one if it is a real lead event on that site.
  const unmapped = Object.entries(seen)
    .filter(([n]) => !map[n] && !/^(page_view|session_start|first_visit|user_engagement|scroll|click|file_download|video_|view_|form_start)/.test(n))
    .map(([event, count]) => ({ event, count }))
    .sort((a, b) => b.count - a.count);
  return { rows: Object.values(days).sort((a, b) => a.day.localeCompare(b.day)), unmapped };
}
