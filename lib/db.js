...
function driver() {
  ...
  if (!_neon) {
    ...
    // cache: "no-store" matters. Next.js patches global fetch and will happily
    // serve a cached copy of an identical Neon HTTP query (same SQL, same
    // params), so a dashboard reloaded after new events would show the old
    // numbers. Verified 2026-09-19.
    _neon = neon(url, { fetchOptions: { cache: "no-store" } });
  }
